...
Username/password accounts backed by a per-user on-chain PDA. Custodial: the password is never a signing secret, only a login credential (see project context — architecture Q1). Every write is signed server-side by the system admin wallet.
