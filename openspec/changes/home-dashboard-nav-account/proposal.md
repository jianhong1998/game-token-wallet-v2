...
- Home's game rows are non-interactive for now (no per-game detail page yet — tracked as a separate future ticket).
...
- Affected frontend: `apps/frontend/src/app/(app)/page.tsx` (rewritten — Home dashboard), `apps/frontend/src/app/(app)/games/page.tsx` (deleted, plus colocated test), `apps/frontend/src/app/(app)/account/page.tsx` (new), `apps/frontend/src/app/(app)/layout.tsx` (adds bottom nav), new `components/` bottom-nav component.
- Depends on ticket 006 (join public game) for `players: Vec<Pubkey>` membership data and the "Browse" route target.
- Depends on change `remove-noop-relocate-dashboard` (019) landing first, since it relocates the dashboard placeholder to `/`.
